...
import com.tzg.wheelrecyclerView.R;
...
public class WRView extends RecyclerView {
    ...
    //默认参数
    private final int DEFAULT_WIDTH = Tool.dp2px(160);

    private final int DEFAULT_ITEM_HEIGHT = Tool.dp2px(50);
    ...
    private final int DEFAULT_SELECT_TEXT_SIZE = Tool.sp2px(14);

    private final int DEFAULT_UNSELECT_TEXT_SIZE = Tool.sp2px(14);

    private final int DEFAULT_OFFSET = 1;

    private final int DEFAULT_DIVIDER_WIDTH = ViewGroup.LayoutParams.MATCH_PARENT;

    private final int DEFAULT_DIVIDER_HEIGHT = Tool.dp2px(1);
    ...
    public WRView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.WRView);

        mItemHeight = (int) ta.getDimension(R.styleable.WRView_itemHeight, DEFAULT_ITEM_HEIGHT);
        mSelectTextColor = ta.getColor(R.styleable.WRView_selectTextColor, DEFAULT_SELECT_TEXT_COLOR);
        mUnselectTextColor = ta.getColor(R.styleable.WRView_unselectTextColor, DEFAULT_UNSELECT_TEXT_COLOR);
        mSelectTextSize = ta.getDimension(R.styleable.WRView_selectTextSize, DEFAULT_SELECT_TEXT_SIZE);
        mUnselectTextSize = ta.getDimension(R.styleable.WRView_unselectTextSize, DEFAULT_UNSELECT_TEXT_SIZE);
        mOffset = ta.getInteger(R.styleable.WRView_wheelOffset, DEFAULT_OFFSET);
        mDividerWidth = ta.getDimension(R.styleable.WRView_dividerWidth, DEFAULT_DIVIDER_WIDTH);
        mDividerHeight = ta.getDimension(R.styleable.WRView_dividerHeight, DEFAULT_DIVIDER_HEIGHT);
        mDividerColor = ta.getColor(R.styleable.WRView_dividerColor, DEFAULT_DIVIVER_COLOR);
